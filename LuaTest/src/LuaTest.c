...
int sayHello(lua_State * L){
	...
	lua_pushstring(L,"sayhello success");
	return 1;
}
...
//	lua_register(L,"sayHello",sayHello);
	lua_newtable(L);
	luaL_setfuncs(L,xx,0);
	lua_setglobal(L,"CCDi");

	dofile(L,"app.lua");
//	dostring(L,"print(\"Hello Lua\")",NULL);
...
//
//	if(lua_isstring(L,-1)){
//		printf("%s child is %s",lua_tostring(L,2),lua_tostring(L,-1));
//	}
//	lua_pop(L,4);
...
//	lua_pushstring(L,"along");
//	lua_pushnumber(L,25);
//	lua_call(L,2,2);
//
//	printf("%s %g",lua_tostring(L,1),lua_tonumber(L,2));
//	lua_pop(L,2);
